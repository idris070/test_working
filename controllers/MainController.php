...
class MainController extends BehaviorsController
{
    ...
    public function actionNews_add(){
      ...
      $news = $model->news_add();
      return $this->render('news_add',['model'=>$model]);
    }
    public function actionViewsnews(){
      $news = News::find()->all();
      $users = User::find()->all();
      return $this->render('viewsnews',['news'=>$news,'users'=>$users]);
    }
    public function actionModer()
    {
      if (isset($_GET['publication'])){
      News::updateAll(['status' => 1], "id = $_GET[publication]");
    }
      if (isset($_GET['on'])){
      User::updateAll(['status' => 0], "id = $_GET[on]");
    }
    if (isset($_GET['off'])){
      User::updateAll(['status' => 10], "id = $_GET[off]");
    }
      $users = User::find()->all();
      $news = News::find()->all();
      return $this->render('moder',['users'=>$users,'news'=>$news]);
    }
